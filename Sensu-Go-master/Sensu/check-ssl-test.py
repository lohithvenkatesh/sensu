...
def critical(hostinfo):
    ...
    exdays = daysi
    if exdays.days <= 10:
        print("CRITICAL : {} SSL certificate will expire on {} days: {} left".format(hostinfo.hostname,
                                                                                    hostinfo.cert.not_valid_after,
                                                                                    daysi.days))
    elif exdays.days <=30:
        print("WARNING : {} SSL certificate will expire on {} days: {} left".format(hostinfo.hostname,
                                                                                     hostinfo.cert.not_valid_after,
                                                                                     daysi.days))
    else:
        print("OK : ALL SSL certificate are upto date..")
